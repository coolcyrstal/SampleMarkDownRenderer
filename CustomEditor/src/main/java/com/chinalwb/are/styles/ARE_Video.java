...
public class ARE_Video implements IARE_Style {
    ...
    private void openVideoChooser() {
        ...
        ((AppCompatActivity) this.mContext).startActivityForResult(intent, ARE_Toolbar.REQ_VIDEO_CHOOSE);
    }
...
//        this.mEditText.useSoftwareLayerOnAndroid8();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M
                && ((AppCompatActivity) mContext).checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE)
                != PackageManager.PERMISSION_GRANTED) {
            //申请授权
            ((AppCompatActivity) mContext).requestPermissions(new String[]{Manifest.permission.READ_EXTERNAL_STORAGE}, ARE_Toolbar.REQ_VIDEO);
            return;
        }
